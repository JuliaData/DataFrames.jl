...
perf_test["sum(removeNA(dv)): DataVector with no NA's"] = () -> f2(dv)
perf_test["sum(*removeNA(dv)): DataVector with no NA's"] = () -> f3(dv)

perf_test["sum(dvna): DataVector with NA's"] = () -> f4(dv)
perf_test["sum(removeNA(dvna)): DataVector with NA's"] = () -> f5(dv)
perf_test["sum(*removeNA(dvna)): DataVector with NA's"] = () -> f6(dv)

perf_test["mean(v): Vector with no NA's"] = () -> f4(v)
perf_test["mean(dv): DataVector with no NA's"] = () -> f4(dv)
perf_test["mean(removeNA(dv)): DataVector with no NA's"] = () -> f5(dv)
perf_test["mean(*removeNA(dv)): DataVector with no NA's"] = () -> f6(dv)

perf_test["mean(dvna): DataVector with NA's"] = () -> f4(dv)
perf_test["mean(removeNA(dvna)): DataVector with NA's"] = () -> f5(dv)
perf_test["mean(*removeNA(dvna)): DataVector with NA's"] = () -> f6(dv)
