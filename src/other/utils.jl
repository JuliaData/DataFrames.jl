...
If `AsTable` is used in `destination` position it means that the result of the operation
is a vector of containers that be expanded into multiple columns using `keys` to get column names.
