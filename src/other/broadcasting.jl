...
function Base.copy(bc::Base.Broadcast.Broadcasted{DataFrameStyle})
    ...
    colnames = unique!(Symbol[_names(df) for df in bcf.args if df isa AbstractDataFrame])
...
function Base.copyto!(df::AbstractDataFrame, bc::Base.Broadcast.Broadcasted)
    bcf = Base.Broadcast.flatten(bc)
    colnames = unique!(Symbol[_names(x) for x in bcf.args if x isa AbstractDataFrame])
