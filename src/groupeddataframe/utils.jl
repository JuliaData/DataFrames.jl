...
function hashrows_col!(h::Vector{UInt},
    ...
    @spawn_for_chunks 10_000_000 for i in eachindex(h)
        @inbounds begin
            el = v[i]
            h[i] = hash(el, h[i])
            if length(n) > 0
...
function hashrows_col!(h::Vector{UInt},
    ...
    if firstcol && Int64(2) * length(rp) < length(v)
        ...
        @spawn_for_chunks 10_000_000 for i in eachindex(hashes)
        ...
        @spawn_for_chunks 10_000_000 for i in eachindex(h, v)
