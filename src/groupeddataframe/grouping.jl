...
If the value returned  by `f` contains columns with the same names as the grouping columns,
...
Additional columns in groups are created using the following rules:
- A single value gives a a single additional column on and one row per group.
- A named tuple of single values or a [`DataFrameRow`](@ref) gives a one additional column
