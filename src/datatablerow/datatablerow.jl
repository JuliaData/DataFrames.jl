...
function Base.getindex(r::DataFrameRow, idx::AbstractArray)
    return DataFrameRow(r.dt[idx], r.row)
end
...
Base.names(r::DataFrameRow) = names(r.dt)
_names(r::DataFrameRow) = _names(r.dt)

Base.view(r::DataFrameRow, c) = DataFrameRow(r.dt[[c]], r.row)

index(r::DataFrameRow) = index(r.dt)

Base.length(r::DataFrameRow) = size(r.dt, 2)

Base.endof(r::DataFrameRow) = size(r.dt, 2)
...
Base.convert(::Type{Array}, r::DataFrameRow) = convert(Array, r.dt[r.row,:])
...
function isequal_row(dt1::AbstractDataFrame, r1::Int, dt2::AbstractDataFrame, r2::Int)
    if dt1 === dt2
        if r1 == r2
            return true
        end
