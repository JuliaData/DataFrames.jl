...
If more than one argument is passed then requesting duplicate column names in target
data frame are accepted (e.g. `select(df, :a, :, :a)` is allowed) and they get deduplicated.
The exception is for column renaming and transformation operations,
where it is not alloweded to rename/transform into the same column name,
e.g. `select(df, :a, :a => sin => :a)` or `select(df, :a, :a => :a)` is allowed but
`select(df, :a => :a, :a => sin => :a)` is not allowed.
