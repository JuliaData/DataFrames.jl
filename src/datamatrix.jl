...
for (f, basef) in ((:dmzeros, :zeros), (:dmones, :ones), (:dmeye, :eye))
    @eval begin
        ($f)(n::Int64, p::Int64) = DataArray(($basef)(n, p), falses(n, p))
        ($f)(t::Type, n::Int64, p::Int64) = DataArray(($basef)(t, n, p), falses(n, p))
    end
end
...
for (f, basef) in ((:dmfalses, :falses), (:dmtrues, :trues))
    @eval begin
        ($f)(n::Int64, p::Int64) = DataVec(($basef)(n, p), falses(n, p))
    end
end
