...
function split_separated_line{T <: String}(line::T,
  ...
  for chr in line
    ...
    if inside_quotes
      ...
      if chr == quotation_character
        ...
        if chr == separator
          push(items, bytestring(current_item[1:(i - 1)]))
          i = 0
          current_item = Array(Uint8, strlen(line))
        else
          current_item[i] = chr
  ...
  push(items, bytestring(current_item[1:i]))
  return items
end
...
function determine_metadata{T <: String}(filename::String,
  ...
  if header
    ...
    column_names = convert(Array{ASCIIString}, fields)
    i = 1
  else
...
function read_minibatch(file::IOStream,
                        separator::Char,
                        quotation_character::Char,
  ...
  text_data = Array(ASCIIString, minibatch_size, p)
  ...
  for j in 1:p
    ...
    elseif column_types[j] == ASCIIString
      values = convert(Array{ASCIIString}, text_data[1:i, j])
    else
...
function read_table(file::IOStream,
                    separator::Char,
                    quotation_character::Char,
                    missingness_indicators::Vector,
                    header::Bool,
