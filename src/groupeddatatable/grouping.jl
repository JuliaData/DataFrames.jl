...
"""
function groupby{T}(dt::AbstractDataFrame, cols::Vector{T}; sort::Bool = false)
    sdt = dt[cols]
    ...
    GroupedDataFrame(dt, cols, dt_groups.rperm,
...
by(dt, :a, d -> DataFrame(c_sum = sum(d[:c]), c_mean = mean(Nulls.skip(d[:c]))))
by(dt, :a, d -> DataFrame(c = d[:c], c_mean = mean(Nulls.skip(d[:c]))))
by(dt, [:a, :b]) do d
