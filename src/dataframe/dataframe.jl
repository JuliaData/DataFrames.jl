...
Alternatively a dictionary can be passed to the constructor in which case its
entries are considered to define the column name and column value pairs with the
exception that in this case column names can be of any type and will be
generated by calling the `Symbol` function on them. Note that if the dictionary
is a `Dict` then column names will be sorted in the returned `DataFrame`.
