...
All columns in `columns` should be `AbstractVector`s and have the same length.
...
an `AbstractArray`, in which case the passed value is implicitly broadcasted.
...
- `col` : an `AbstractVector` giving the contents of the new column or a value of any
  other type that is not `AbstractArray` which will be implicitly broadcasted
