...
a new vector of the appropriate length. As a particular rule if a column is
`Ref` or `0`-dimentional `AbstractArray` then it is allowed and in both cases
the value stored in the container is automatically repeated.
...
- `col` : an `AbstractVector` giving the contents of the new column or a value of any
  ...
  As a particular rule if `col` is `Ref` or `0`-dimentional `AbstractArray` then it is
  allowed and in both cases the value stored in the container is used for filling.
