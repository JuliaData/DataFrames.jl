...
```julia
stack(dt::AbstractDataFrame, [measure_vars], [id_vars];
      variable_name::Symbol=:variable, value_name::Symbol=:value)
melt(dt::AbstractDataFrame, [id_vars], [measure_vars];
     variable_name::Symbol=:variable, value_name::Symbol=:value)
```

### Arguments

* `dt` : the AbstractDataFrame to be stacked
...
"""
function stack(dt::AbstractDataFrame, measure_vars::Vector{Int},
    ...
    DataFrame(Any[Compat.repeat(_names(dt)[measure_vars], inner=nrow(dt)),   # variable
                  vcat([dt[c] for c in measure_vars]...),                    # value
                  [Compat.repeat(dt[c], outer=N) for c in id_vars]...],      # id_var columns
              cnames)
end
function stack(dt::AbstractDataFrame, measure_var::Int, id_var::Int;
               variable_name::Symbol=:variable, value_name::Symbol=:value)
    stack(dt, [measure_var], [id_var];
          variable_name=variable_name, value_name=value_name)
end
function stack(dt::AbstractDataFrame, measure_vars::Vector{Int}, id_var::Int;
               variable_name::Symbol=:variable, value_name::Symbol=:value)
    stack(dt, measure_vars, [id_var];
          variable_name=variable_name, value_name=value_name)
end
function stack(dt::AbstractDataFrame, measure_var::Int, id_vars::Vector{Int};
               variable_name::Symbol=:variable, value_name::Symbol=:value)
    stackdt(dt, [measure_var], id_vars;
            variable_name=variable_name, value_name=value_name)
end
function stack(dt::AbstractDataFrame, measure_vars, id_vars;
               variable_name::Symbol=:variable, value_name::Symbol=:value)
    stack(dt, index(dt)[measure_vars], index(dt)[id_vars];
          variable_name=variable_name, value_name=value_name)
end
# no vars specified, by default select only numeric columns
numeric_vars(dt::AbstractDataFrame) =
    [T <: AbstractFloat || (T >: Null && Nulls.T(T) <: AbstractFloat)
     for T in eltypes(dt)]

function stack(dt::AbstractDataFrame, measure_vars = numeric_vars(dt);
               variable_name::Symbol=:variable, value_name::Symbol=:value)
...
`stack`.
"""
function melt(dt::AbstractDataFrame, id_vars::@compat(Union{Int,Symbol});
              variable_name::Symbol=:variable, value_name::Symbol=:value)
    melt(dt, [id_vars]; variable_name=variable_name, value_name=value_name)
end
function melt(dt::AbstractDataFrame, id_vars;
              variable_name::Symbol=:variable, value_name::Symbol=:value)
    id_inds = index(dt)[id_vars]
    stack(dt, setdiff(1:ncol(dt), id_inds), id_inds;
          variable_name=variable_name, value_name=value_name)
end
function melt(dt::AbstractDataFrame, id_vars, measure_vars;
              variable_name::Symbol=:variable, value_name::Symbol=:value)
    stack(dt, measure_vars, id_vars; variable_name=variable_name,
          value_name=value_name)
end
melt(dt::AbstractDataFrame; variable_name::Symbol=:variable, value_name::Symbol=:value) =
...
```julia
unstack(dt::AbstractDataFrame, rowkey, colkey, value)
unstack(dt::AbstractDataFrame, colkey, value)
unstack(dt::AbstractDataFrame)
```

### Arguments

* `dt` : the AbstractDataFrame to be unstacked
...
"""
function unstack(dt::AbstractDataFrame, rowkey::Int, colkey::Int, value::Int)
...
function unstack(dt::AbstractDataFrame, colkey::Int, value::Int)
    ...
    dt2 = DataFrame(Any[similar_nullable(valuecol, Nrow) for i in 1:Ncol], map(Symbol, levels(keycol)))
    nowarning = true
    for k in 1:nrow(dt)
        j = Int(CategoricalArrays.order(keycol.pool)[keycol.refs[k]])
        i = rowkey[k]
...
unstack(dt::AbstractDataFrame) = unstack(dt, :id, :variable, :value)
...
```julia
stackdt(dt::AbstractDataFrame, [measure_vars], [id_vars];
        variable_name::Symbol=:variable, value_name::Symbol=:value)
meltdt(dt::AbstractDataFrame, [id_vars], [measure_vars];
       variable_name::Symbol=:variable, value_name::Symbol=:value)
```

### Arguments

* `dt` : the wide AbstractDataFrame
...
"""
function stackdt(dt::AbstractDataFrame, measure_vars::Vector{Int},
                 id_vars::Vector{Int}; variable_name::Symbol=:variable,
                 value_name::Symbol=:value)
    N = length(measure_vars)
    cnames = names(dt)[id_vars]
    insert!(cnames, 1, value_name)
    insert!(cnames, 1, variable_name)
    DataFrame(Any[RepeatedVector(_names(dt)[measure_vars], nrow(dt), 1),   # variable
                  StackedVector(Any[dt[:,c] for c in measure_vars]),     # value
                  [RepeatedVector(dt[:,c], 1, N) for c in id_vars]...],     # id_var columns
              cnames)
end
function stackdt(dt::AbstractDataFrame, measure_var::Int, id_var::Int;
                 variable_name::Symbol=:variable, value_name::Symbol=:value)
    stackdt(dt, [measure_var], [id_var]; variable_name=variable_name,
            value_name=value_name)
end
function stackdt(dt::AbstractDataFrame, measure_vars, id_var::Int;
                 variable_name::Symbol=:variable, value_name::Symbol=:value)
    stackdt(dt, measure_vars, [id_var]; variable_name=variable_name,
            value_name=value_name)
end
function stackdt(dt::AbstractDataFrame, measure_var::Int, id_vars;
                 variable_name::Symbol=:variable, value_name::Symbol=:value)
    stackdt(dt, [measure_var], id_vars; variable_name=variable_name,
            value_name=value_name)
end
function stackdt(dt::AbstractDataFrame, measure_vars, id_vars;
                 variable_name::Symbol=:variable, value_name::Symbol=:value)
    stackdt(dt, index(dt)[measure_vars], index(dt)[id_vars];
            variable_name=variable_name, value_name=value_name)
end
function stackdt(dt::AbstractDataFrame, measure_vars = numeric_vars(dt);
                 variable_name::Symbol=:variable, value_name::Symbol=:value)
...
"""
A stacked view of a DataFrame (long format); see `stackdt`
"""
function meltdt(dt::AbstractDataFrame, id_vars; variable_name::Symbol=:variable,
                value_name::Symbol=:value)
    id_inds = index(dt)[id_vars]
    stackdt(dt, setdiff(1:ncol(dt), id_inds), id_inds;
            variable_name=variable_name, value_name=value_name)
end
function meltdt(dt::AbstractDataFrame, id_vars, measure_vars;
                variable_name::Symbol=:variable, value_name::Symbol=:value)
    stackdt(dt, measure_vars, id_vars; variable_name=variable_name,
            value_name=value_name)
end
meltdt(dt::AbstractDataFrame; variable_name::Symbol=:variable, value_name::Symbol=:value) =
