...
immutable DataFrameJoiner{DT1<:AbstractDataFrame, DT2<:AbstractDataFrame}
    ...
    function DataFrameJoiner{DT1, DT2}(dtl::DT1, dtr::DT2, on::Union{Symbol,Vector{Symbol}}) where {DT1, DT2}
        on_cols = isa(on, Symbol) ? [on] : on
        new(dtl, dtr, dtl[on_cols], dtr[on_cols], on_cols)
    end
end

DataFrameJoiner{DT1<:AbstractDataFrame, DT2<:AbstractDataFrame}(dtl::DT1, dtr::DT2, on::Union{Symbol,Vector{Symbol}}) =
    DataFrameJoiner{DT1,DT2}(dtl, dtr, on)
...
function compose_joined_table(joiner::DataFrameJoiner, kind::Symbol,
    ...
    res = DataFrame(cols, vcat(names(joiner.dtl), names(dtr_noon)))
...
function update_row_maps!(left_table::AbstractDataFrame,
    ...
    init_map(dt::AbstractDataFrame, init::Bool) = init ?
...
```julia
join(dt1::AbstractDataFrame,
     dt2::AbstractDataFrame;
...
* `dt1`, `dt2` : the two AbstractDataFrames to be joined
...
"""
function Base.join(dt1::AbstractDataFrame,
                   dt2::AbstractDataFrame;
                   on::Union{Symbol, Vector{Symbol}} = Symbol[],
                   kind::Symbol = :inner)
    if kind == :cross
    ...
    joiner = DataFrameJoiner(dt1, dt2, on)
...
function crossjoin(dt1::AbstractDataFrame, dt2::AbstractDataFrame)
