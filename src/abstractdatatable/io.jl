...
function printtable(dt::AbstractDataFrame;
                    header::Bool = true,
                    separator::Char = ',',
                    quotemark::Char = '"',
                    nastring::AbstractString = "null")
...
@compat function Base.show(io::IO, ::MIME"text/html", dt::AbstractDataFrame)
    cnames = _names(dt)
...
function Base.show(io::IO, ::MIME"text/latex", dt::AbstractDataFrame)
    nrows = size(dt, 1)
    ncols = size(dt, 2)
    cnames = _names(dt)
...
@compat function Base.show(io::IO, ::MIME"text/csv", dt::AbstractDataFrame)
    printtable(io, dt, true, ',')
end

@compat function Base.show(io::IO, ::MIME"text/tab-separated-values", dt::AbstractDataFrame)
    printtable(io, dt, true, '\t')
end
...
DataFrameStream(dt::DataFrame) = DataFrameStream(Tuple(dt.columns), string.(names(dt)))

# DataFrame Data.Source implementation
function Data.schema(dt::DataFrame)
