...
DataFrame  6 observations of 3 variables
  A: DataVectortortor{Float64}(6) [2.5,3.6,3.5,4.5]
  B: PooledDataVectortortor{ASCIIString}(6) ["One","One","Two","Two"]
  C: DataVectortortor{Float64}(6) [3.0,5.0,3.0,5.0]
...
julia> df[:( A .> 4.0 )]
no method isless(Float64,DataVectortortor{Float64})
...
DataFrame  6 observations of 5 variables
  ...
  B: PooledDataVectortortor{ASCIIString}(6) ["One","One","Two","Two"]
  C: DataVectortortor{Float64}(6) [3.0,5.0,3.0,5.0]
  D: DataVectortortor{Float64}(6) [5.5,8.6,6.5,9.5]
  E: DataVectortortor{Float64}(6) [26.5,27.6,27.5,28.5]
...
julia> # cut makes a PooledDataVectortortor that is like R's factors, but

julia> # PooledDataVectortortors can contain more than just strings. Here's

julia> # the internal structure of a PooledDataVectortortor:

julia> idump(df3["ct"])
PooledDataVectortortor{ASCIIString} 
