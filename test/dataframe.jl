...
@testset "vcat with indicator" begin
    ...
    for col in [:source, "source"]
        @test vcat(df1, df2, df3, df4, cols=:union, indicator=col) ≅
              vcat(df1, df2, df3, df4, cols=:union, indicator=col => [1, 2, 3, 4]) ≅
              DataFrame(source=[1, 1, 1, 2, 2, 2, 3, 3, 3],
                        A=1:9, B=[1:6; fill(missing, 3)],
                        C=[fill(missing, 6); 7:9])
        res = vcat(df1, df2, df3, df4, cols=:union, indicator=col => categorical(-4:-1))
        @test res ≅ DataFrame(source=[-4, -4, -4, -3, -3, -3, -2, -2, -2],
                              A=1:9, B=[1:6; fill(missing, 3)],
                              C=[fill(missing, 6); 7:9])
        @test res.source isa CategoricalVector
    end

    @test_throws TypeError vcat(df1, df2, df3, df4, cols=:union, indicator=1)
    @test_throws TypeError vcat(df1, df2, df3, df4, cols=:union, indicator=:a => 1)
    @test_throws ArgumentError vcat(df1, df2, df3, df4, cols=:union, indicator=:C)
    @test_throws ArgumentError vcat(df1, df2, df3, df4, cols=:union, indicator=:a => [1])
end
