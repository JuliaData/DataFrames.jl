...
@testset "column conversions" begin
    ...
    tmpcol = df[!, 1]
    disallowmissing!(df, 1, skip=true)
    @test df[!, 1] === tmpcol
    df[1,1] = 1
    ...
    tmpcol = df[!, 1]
    disallowmissing!(df, Not(Not(1)), skip=true)
    @test df[!, 1] === tmpcol
    df[1,1] = 1
    ...
    for sm in [true, false]
        ...
        @test disallowmissing!(df, [1,2], skip=sm) === df
        @test isa(df[!, 1], Vector{Int}) && isa(df[!, 2], Vector{Int})

        df = DataFrame([collect(1:10), collect(1:10)])
        @test allowmissing!(df, Not(Not([1,2]))) === df
        @test isa(df[!, 1], Vector{Union{Int, Missing}}) && isa(df[!, 2], Vector{Union{Int, Missing}})
        @test disallowmissing!(df, Not(Not([1,2])), skip=sm) === df
        @test isa(df[!, 1], Vector{Int}) && isa(df[!, 2], Vector{Int})
        ...
        @test isa(df[!, 1], Vector{Union{Int, Missing}}) && isa(df[!, 2], Vector{Union{Int, Missing}})
        @test_throws BoundsError disallowmissing!(df, [true], skip=sm)
        @test disallowmissing!(df, [true,true], skip=sm) === df
        @test isa(df[!, 1], Vector{Int}) && isa(df[!, 2], Vector{Int})

        df = DataFrame([collect(1:10), collect(1:10)])
        @test allowmissing!(df) === df
        @test isa(df[!, 1], Vector{Union{Int, Missing}}) && isa(df[!, 2], Vector{Union{Int, Missing}})
        @test disallowmissing!(df, skip=sm) === df
        @test isa(df[!, 1], Vector{Int}) && isa(df[!, 2], Vector{Int})
        ...
        @test isa(df[!, 1], Vector{Union{Int, Missing}}) && isa(df[!, 2], Vector{Union{Int, Missing}})
        @test disallowmissing!(df, r"", skip=sm) === df
        @test isa(df[!, 1], Vector{Int}) && isa(df[!, 2], Vector{Int})

        df = DataFrame([collect(1:10), collect(1:10)])
        @test allowmissing!(df, Not(1:0)) === df
        @test isa(df[!, 1], Vector{Union{Int, Missing}}) && isa(df[!, 2], Vector{Union{Int, Missing}})
        @test disallowmissing!(df, Not(1:0), skip=sm) === df
        @test isa(df[!, 1], Vector{Int}) && isa(df[!, 2], Vector{Int})
    end
    ...
    for sm in [true, false]
        ...
        @test disallowmissing!(df, :c, skip=sm) === df
        @test eltype(df.b) == Union{Int, Missing}
        ...
        @test disallowmissing!(df, [true, false, false], skip=sm) === df
    ...
    df = DataFrame(x=[1], y = Union{Int,Missing}[1], z=[missing])
    disallowmissing!(df, 2:3, skip=true)
    @test eltype(df.x) == Int
    @test eltype(df.y) == Int
    @test eltype(df.z) == Missing
end
...
@testset "test disallowmissing" begin
    df = DataFrame(x=Union{Int,Missing}[1,2,3],
                   ...
                   z=[1,2,3])
    for x in [df, view(df, :, :)], sm in [true, false]
        y = disallowmissing(x, skip=sm)
        ...
        for colsel in [:, names(x), [1,2,3], [true,true,true], r"", Not(r"a")]
            y = disallowmissing(x, colsel, skip=sm)
            @test y isa DataFrame
            @test x == y
            @test x.x !== y.x
            @test x.y !== y.y
    ...
    df2 = disallowmissing(df, 2:3, skip=true)
    @test eltype(df2.x) == Int
    @test eltype(df2.y) == Int
    @test eltype(df2.z) == Missing
end
