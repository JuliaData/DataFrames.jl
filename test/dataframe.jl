...
@testset "keepat!" begin
    ...
    @test_throws BoundsError keepat!(df, [10])
    @test_throws BoundsError keepat!(df, 0)
    @test_throws BoundsError keepat!(df, [0])
    ...
    df = DataFrame(a=[1, 2, 3], b=[3, 2, 1])
    @test keepat!(df, [3, 2]) == DataFrame(a=[2, 3], b=[2, 1])
    df = DataFrame(a=[1, 2, 3], b=[3, 2, 1])
    @test keepat!(df, [2, 2]) == DataFrame(a=2, b=2)
    df = DataFrame(a=[1, 2, 3], b=[3, 2, 1])
    ...
    for v in (1, [1], 1:1, [true, false, false])
        x = [1, 2, 3]
        df = DataFrame(x=x)
        @test keepat!(df, v) == DataFrame(x=[1])
        @test x == [1, 2, 3]
    end
