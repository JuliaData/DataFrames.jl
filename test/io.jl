...
@testset "LaTeX export" begin
    ...
    @test str == """
        ...
        """
    ioc = IOContext(IOBuffer(), :displaysize=>(10, 10), :limit=>false)
    show(ioc, MIME("text/latex"), df, missingstring="")
    str = String(take!(ioc.io))
    @test str == """
        \\begin{tabular}{r|cccccc}
        \t& A & B & C & D & E & F\\\\
...
@testset "csv/tsv output" begin
    ...
    show(io, MIME("text/csv"), df, missingstring="-")
    str = String(take!(io))
    @test str == """
    "a","b"
    1,"1.0"
    2,-
    """    
    show(io, MIME("text/tab-separated-values"), df, missingstring="-")
    str = String(take!(io))
    @test str == """
    "a"\t"b"
    1\t"1.0"
