module TestDataFrame
    ...
    dt = DataFrame(a = Union{Int, Null}[2, 3],
    ...
    dt = DataFrame(a=Union{Int, Null}[1, 2], b=Union{Float64, Null}[3.0, 4.0])
    ...
    dt = DataFrame(a=[1, 2], b=[3.0, 4.0])
    dt2 = DataFrame(b=["a", "b"], c=[:c, :d])
    @test merge!(dt, dt2) == dt
    @test dt == DataFrame(a=[1, 2], b=["a", "b"], c=[:c, :d])

    #test_group("Empty DataFrame constructors")
    dt = DataFrame(Union{Int, Null}, 10, 3)
    @test size(dt, 1) == 10
    @test size(dt, 2) == 3
    ...
    dt = DataFrame([Union{Int, Null}, Union{Float64, Null}, Union{String, Null}], 100)
    @test size(dt, 1) == 100
    @test size(dt, 2) == 3
    @test typeof(dt[:, 1]) == Vector{Union{Int, Null}}
    ...
    dt = DataFrame([Union{Int, Null}, Union{Float64, Null}, Union{String, Null}],
                   [:A, :B, :C], 100)
    @test size(dt, 1) == 100
    @test size(dt, 2) == 3
    @test typeof(dt[:, 1]) == Vector{Union{Int, Null}}
    ...
    dt = convert(DataFrame, zeros(10, 5))
    @test size(dt, 1) == 10
    @test size(dt, 2) == 5
    @test typeof(dt[:, 1]) == Vector{Float64}

    dt = convert(DataFrame, ones(10, 5))
    @test size(dt, 1) == 10
    @test size(dt, 2) == 5
    @test typeof(dt[:, 1]) == Vector{Float64}

    dt = convert(DataFrame, eye(10, 5))
    @test size(dt, 1) == 10
    @test size(dt, 2) == 5
    @test typeof(dt[:, 1]) == Vector{Float64}
    ...
    dt = DataFrame(Column = [:A])
    dt[1, :Column] = "Testing"

    # zero-row DataFrame and subDataFrame test
    dt = DataFrame(x=[], y=[])
    @test nrow(dt) == 0
    dt = DataFrame(x=[1:3;], y=[3:5;])
    sdt = view(dt, dt[:x] .== 4)
    @test size(sdt, 1) == 0
    ...
    # push!(dt, row)
    dt=DataFrame( first=[1,2,3], second=["apple","orange","pear"] )

    dtb= DataFrame( first=[1,2], second=["apple","orange"] )
    push!(dtb, Any[3,"pear"])
    @test dt == dtb
    ...
    dt=DataFrame( first=[1,2,3], second=["apple","orange","banana"] )
    dtb= DataFrame( first=[1,2], second=["apple","orange"] )
    push!(dtb, Dict("first"=>3, "second"=>"banana"))
    @test dt == dtb

    dt0= DataFrame( first=[1,2], second=["apple","orange"] )
    dtb= DataFrame( first=[1,2], second=["apple","orange"] )
    ...
    # delete!
    dt = DataFrame(a=1, b=2, c=3, d=4, e=5)
    ...
    @test dt == DataFrame(a=[2], b=[4.0])

    dt = DataFrame(a=[1, 2], b=[3.0, 4.0])
    @test deleterows!(dt, 2) === dt
    @test dt == DataFrame(a=[1], b=[3.0])

    dt = DataFrame(a=[1, 2, 3], b=[3.0, 4.0, 5.0])
    ...
    dt = DataFrame(a=Union{Int, Null}[1, 2, 3], b=Union{Float64, Null}[3.0, 4.0, 5.0])
    @test deleterows!(dt, 2:3) === dt
    @test dt == DataFrame(a=[1], b=[3.0])
    ...
    dt = DataFrame(Fish = CategoricalArray{Union{String, Null}}(["Bob", "Bob", "Batman", "Batman"]),
    ...
    #The expected output
    dt4 = DataFrame(Fish = Union{String, Null}["XXX", "Bob", "Batman"],
    ...
    dt = DataFrame(A = 1:10, B = 'A':'J')
    @test !(dt[:,:] === dt)

    @test append!(DataFrame(A = 1:2, B = 1:2), DataFrame(A = 3:4, B = 3:4)) == DataFrame(A=1:4, B = 1:4)
    dt = DataFrame(A = Vector{Union{Int, Null}}(1:3), B = Vector{Union{Int, Null}}(4:6))
    ...
    @testset "duplicate entries in unstack warnings" begin
        dt = DataFrame(id=Union{Int, Null}[1, 2, 1, 2], variable=["a", "b", "a", "b"], value=[3, 4, 5, 6])
        ...
        dt = DataFrame(id=1:2, variable=["a", "b"], value=3:4)
        @static if VERSION >= v"0.6.0-dev.1980"
    ...
    @testset "description" begin
        dt = DataFrame(A = 1:10)
        @test head(dt) == DataFrame(A = 1:6)
        @test head(dt, 1) == DataFrame(A = 1)
        @test tail(dt) == DataFrame(A = 5:10)
        @test tail(dt, 1) == DataFrame(A = 10)
    end

    @testset "misc" begin
        dt = DataFrame(Any[collect('A':'C')])
        @test sprint(dump, dt) == """
        ...
        # @test DataFrames.without(dt, 1) == DataFrame(B = repeat('A':'C', inner=4))
    end

    @testset "column conversions" begin
        dt = DataFrame(Any[collect(1:10), collect(1:10)])
