...
@testset "corner cases of group_reduce" begin
    ...
    gdf = groupby_checked(df, :g)
    @test combine_checked(gdf, :x => sum)[1, 2] == sum(df.x)
    @test eltype(combine_checked(gdf, :x => sum)[!, 2]) === BigInt
    df = DataFrame(g=[1, 1, 1, 1, 1, 1], x=Real[1, 1, typemax(Int), 1, 1, 1])
    gdf = groupby_checked(df, :g)
    @test combine_checked(gdf, :x => sum)[1, 2] == sum(df.x)
    @test eltype(combine_checked(gdf, :x => sum)[!, 2]) === Int
    ...
    gdf = groupby_checked(df, :g)
    @test combine_checked(gdf, :x => sum)[1, 2] isa Missing
    @test eltype(combine_checked(gdf, :x => sum)[!, 2]) === Missing
    @test combine_checked(gdf, :x => sum∘skipmissing) == DataFrame(g=1, x_sum_skipmissing=0)
    @test eltype(combine_checked(gdf, :x => sum∘skipmissing)[!, 2]) === Int
    df = DataFrame(g=[1, 1, 1, 1, 1, 1], x=convert(Vector{Union{Int, Missing}}, fill(missing, 6)))
    gdf = groupby_checked(df, :g)
    @test combine_checked(gdf, :x => sum)[1, 2] isa Missing
    @test eltype(combine_checked(gdf, :x => sum)[!, 2]) === Missing
    @test combine_checked(gdf, :x => sum∘skipmissing)[1, 2] == 0
    @test eltype(combine_checked(gdf, :x => sum∘skipmissing)[!, 2]) === Int
