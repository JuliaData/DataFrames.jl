...
@testset "the output of unstack" begin
    ...
    levels!(df[!, 1], ["XXX", "Bob", "Batman"])
    levels!(df[!, 2], ["YYY", "Color", "Mass"])
    df2 = unstack(df, :Fish, :Key, :Value, transform=x->string("_", uppercase(x), "_"))
    df3 = unstack(df, :Key, :Value, transform=x->string("_", uppercase(x), "_"))
    df4 = DataFrame(Fish = Union{String, Missing}["Bob", "Batman"],
    ...
    @test_throws ArgumentError unstack(stack(DataFrame(rand(10, 10))),
                                  :id, :variable, :value)
    @test_throws TypeError unstack(df, :Key, :Value, transform=Symbol)
    ...
    wide3 = unstack(long, [:id, :a], :variable, :value,
                    transform=x->string("_", uppercase(string(x)), "_"))
    @test wide3 == w2
