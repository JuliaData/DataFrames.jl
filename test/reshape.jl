...
@testset "the output of unstack" begin
    ...
    @test_throws ArgumentError unstack(stack(DataFrame(rand(10, 10), :gennames)),
                                  :id, :variable, :value)
...
@testset "stack-unstack correctness" begin
    Random.seed!(1234)
    x = DataFrame(rand(100, 50), :gennames)
    x[!, :id] = [1:99; missing]
    x[!, :id2] = string.("a", x[!, :id])
...
@testset "reshape" begin
    ...
    df1 = stack(DataFrame(rand(10,10), :gennames))
    df1[!, :id] = 1:100
    @test size(unstack(df1, :variable, :value)) == (100, 11)
    @test unstack(df1, :variable, :value) ≅ unstack(df1)

    # test empty keycol
    @test_throws ArgumentError unstack(stack(DataFrame(rand(3,2), :gennames)), :variable, :value)
end
...
@testset "test stack eltype" begin
    df = DataFrame(rand(4,5), :gennames)
    sdf = stack(df)
