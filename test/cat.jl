...
@testset "vcat mixed coltypes" begin
    ...
    @test typeof.(eachcol(df, false)) == [Vector{Float64}]
    df = vcat(DataFrame([[1]], [:x]), DataFrame([["1"]], [:x]))
    @test df == DataFrame([[1, "1"]], [:x])
    @test typeof.(eachcol(df, false)) == [Vector{Any}]
    df = vcat(DataFrame([Union{Missing, Int}[1]], [:x]), DataFrame([[1]], [:x]))
    @test df == DataFrame([[1, 1]], [:x])
    @test typeof.(eachcol(df, false)) == [Vector{Union{Missing, Int}}]
    ...
    @test typeof.(eachcol(df, false)) == [Vector{Bool}]
end
